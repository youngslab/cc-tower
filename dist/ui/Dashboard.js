...
export function Dashboard({ sessions, tmuxCount, maxTaskWidth, onSelect, onSend, onPeek, onToggleFavorite, onNewSession, onRefresh, onQuit }) {
    ...
    return (_jsxs(Box, { flexDirection: "column", children: [_jsxs(Box, { children: [_jsx(Text, { bold: true, children: "   " }), _jsx(Text, { bold: true, dimColor: true, children: pad('LABEL', 22) }), _jsx(Text, { bold: true, dimColor: true, children: pad('●', 3) }), _jsx(Text, { bold: true, dimColor: true, children: "GOAL" })] }), sorted.map((session, i) => {
                ...
                return (_jsxs(React.Fragment, { children: [showFavSep && (_jsxs(Text, { dimColor: true, children: ['· · · ·'.repeat(5), " \u2605 favorites"] })), showNonTmuxSep && (_jsxs(Text, { dimColor: true, children: ['· · · ·'.repeat(5), " (monitor-only)"] })), _jsxs(Box, { children: [_jsx(Text, { color: isCursor ? 'cyan' : undefined, bold: isCursor, children: isCursor ? '▸' : ' ' }), _jsx(Text, { color: isCursor ? 'cyan' : undefined, dimColor: !isCursor, children: pad(`${i + 1}`, 3) }), _jsx(Text, { color: isCursor ? 'cyan' : undefined, dimColor: !isCursor && isDim, children: pad(labelText, 22) }), _jsx(Text, { color: isCursor ? 'cyan' : (isDim ? 'gray' : color), children: pad(icon, 3) }), session.label && _jsxs(Text, { color: isCursor ? 'cyan' : 'blue', bold: true, children: ["[", session.label, "] "] }), _jsx(Text, { color: isCursor ? 'cyan' : undefined, dimColor: !isCursor && isDim, children: truncate(session.goalSummary ?? session.contextSummary ?? session.currentTask ?? (session.summaryLoading ? '⟳ summarizing...' : ''), maxTaskWidth - (session.label ? session.label.length + 3 : 0)) })] }), session.status === 'idle' && session.nextSteps && (_jsx(Box, { paddingLeft: 52, children: _jsxs(Text, { color: "yellow", children: ["\u21B3 ", truncate(session.nextSteps, maxTaskWidth)] }) }))] }, session.sessionId));
