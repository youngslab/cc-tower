 ...
 */
export async function generateAllSummaries(sessionId, earlyMessages, recentMessages) {
    const hash = contentHash(earlyMessages + '\x00' + recentMessages);
    const cached = allCache.get(sessionId);
    if (cached && cached.hash === hash)
        return cached.result;
    if (allInflight.has(sessionId))
        return cached?.result;
    allInflight.add(sessionId);
    try {
        const prompt = `Read this dev session. Output ONLY a valid JSON object with these exact keys:\n` +
...
function parseAllSummariesOutput(raw) {
    ...
    try {
        ...
        if (typeof parsed['nextSteps'] === 'string') {
            ...
            if (n.length > 3 && n.toUpperCase() !== 'NONE')
                result.nextSteps = n;
        }
