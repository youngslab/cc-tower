...
export class SessionScanner {
    projectsDir;
    cache = new Map(); // sessionId → scanned
    dirWatermark = new Map(); // slug dir → mtimeMs at last scan
    scanned = false;
    inFlight = null;
    ...
    async scan() {
        ...
        for (const dirent of dirents) {
            ...
            let dirMtime;
            try {
                dirMtime = fs.statSync(dirPath).mtimeMs;
            }
            catch {
                continue;
            }
            if (this.dirWatermark.get(dirent.name) === dirMtime)
    ...
    scanTailForCustomTitle(filePath, fileSize, headCovered) {
