...
import { remoteReadJsonlTail, remoteListPanes } from '../ssh/remote-commands.js';
...
export class Tower extends EventEmitter {
    ...
    remoteStateMachines = new Map();
    hookSidToIdentity = new Map(); // CLAUDE_SESSION_ID → identity (paneId ?? String(pid))
    jsonlPaths = new Map(); // sessionId → jsonlPath
    stopping = false;
    connectionManager;
    remoteDiscovery = null;
    remotePollers = new Map();
    readOnlyDrainTimer = null;
    skipHooks;
    skipColdStart;
    skipSummary;
    readOnly;
    ...
    async start() {
        ...
        this.discovery.on('session-changed', async ({ prev, next }) => {
            ...
            const isResume = (() => { try {
                return fs.statSync(nextJsonl).size > 0;
            }
            catch {
                return false;
            } })();
        ...
        if (this.config.hosts.length > 0) {
            ...
            const remoteHosts = this.config.hosts.map(h => ({
                name: h.name,
                config: { sshTarget: h.ssh, sshOptions: h.ssh_options, claudeDir: h.claude_dir, commandPrefix: h.command_prefix },
            }));
        ...
        try {
            ...
            for (const f of fs.readdirSync(sessionsDir)) {
                ...
                try {
                    ...
                    if (typeof pidJson.sessionId === 'string')
                        liveSessionIds.add(pidJson.sessionId);
                }
                catch { }
            }
        }
    ...
    drainEventQueue() {
        ...
        let lines = [];
        try {
            const raw = execSync(`flock -x "${lockFile}" sh -c 'cat "${queueFile}" && : > "${queueFile}"'`, { encoding: 'utf8', timeout: 3000 });
            lines = raw.split('\n').filter(Boolean);
        }
        catch (err) {
    ...
    static PANE_TITLE_OVERRIDE_GRACE_MS = 30000;
    ...
    async refreshSession(sessionId) {
        ...
        if (match && match.sessionId !== sessionId) {
            ...
            if (session.label !== undefined)
                migrationMeta.label = session.label;
            if (session.tags !== undefined)
                migrationMeta.tags = session.tags;
     ...
     */
    async refreshSessionAfterResume(identity) {
        const session = this.store.get(identity);
        if (!session)
            return;
    ...
    async refreshAllRemoteSummaries(compositeId, config, jsonlPath) {
        if (this.skipSummary)
            return;
    ...
    async stop() {
        this.stopping = true;
        this.releaseLock();
        if (this.readOnlyDrainTimer)
            clearInterval(this.readOnlyDrainTimer);
        this.discovery.stop();
    ...
    async registerSession(info, opts = {}) {
        ...
        fsm.on('inactivity-check', () => {
            ...
            if (!sess || sess.status === 'idle' || sess.status === 'dead')
                return;
            ...
            try {
                ...
                for (let i = lines.length - 1; i >= 0; i--) {
                    ...
                    if (parsed.type === 'user' && parsed.userContent) {
                        ...
                        fsm.transition({ type: 'jsonl', stopReason: null }); // thinking
                        break;
                    }
    ...
    resolveIdentity(hookSid, hookCwd, hookPid, hookEvent, hookPane) {
        ...
        if (cached && this.stateMachines.has(cached))
            return { identity: cached, depth: 0 };
    ...
    handleHookEvent(event) {
        ...
        if (!hookSid || typeof hookSid !== 'string')
            return;
        if (event.pid !== undefined && (typeof event.pid !== 'number' || event.pid < 0))
            return;
        if (event.pane !== undefined && typeof event.pane !== 'string')
            return;
        ...
        if (session) {
            ...
            if (hookSid !== 'unknown' && hookSid !== session.sessionId && isDirectChild) {
                ...
                if (fs.existsSync(newJsonl)) {
                    ...
                    try {
                        let settled = false;
                        const timeout = setTimeout(() => { if (!settled)
                            dirWatcher.close(); }, 60000);
            ...
            if (event.pane && event.pane !== session.paneId) {
                ...
                if (Object.keys(patch).length > 0)
                    this.store.update(oldIdentity, patch);
                ...
                if (oldIdentity !== newIdentity) {
                    ...
                    if (fsm2) {
                        ...
                        this.stateMachines.set(newIdentity, fsm2);
                    }
                    for (const [hSid, id] of this.hookSidToIdentity) {
                        if (id === oldIdentity)
                            this.hookSidToIdentity.set(hSid, newIdentity);
                    }
