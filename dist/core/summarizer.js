export class Summarizer {
    summarize(currentState, event, recentMessages) {
    ...
    summarizeWithContent(currentState, event, content) {
    ...
    extractSummary(event, _recentMessages) {
        if (event.type === 'user') {
    ...
    matchTsErrors(content) {
        const errors = content.match(/error TS\d+/g) || [];
        if (errors.length > 0)
            return `Build: ${errors.length} errors`;
        return null;
    }
    matchCommit(content) {
        const m = content.match(/(?:commit|Committed?:?)\s+([a-f0-9]{7,40})/i);
        if (m)
            return `Committed: ${m[1].slice(0, 7)}`;
        return null;
    }
    matchFileEdits(content) {
        const files = [];
