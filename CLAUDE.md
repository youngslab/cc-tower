...
| 2.8.12 | feat: F12로 picker를 열었을 때 커서가 "지금 보고 있는 세션"이 아니라 이전에 마지막으로 탐색했던 위치(`picker-cursor` 파일)에서 시작하던 문제 — `popmux-go`가 `display-popup` 진입 **전에**(popup 내부에선 `$TMUX_PANE`이 popup 자신의 pane이 되어버림) `tmux display-message -p '#{pane_id}'`로 현재 클라이언트가 보고 있는 pane을 캡처해 `--origin-pane`으로 picker에 전달, App.tsx가 이를 persisted cursor보다 우선해 초기 커서로 사용하도록 수정. `tmux display-message`(타겟 미지정)가 명령 실행 위치가 아니라 "현재 클라이언트가 보는 pane"을 반환한다는 것을 실측으로 확인 — F12 키바인딩이 클라이언트 컨텍스트에서 실행되므로 정확히 원하는 동작과 일치 |
