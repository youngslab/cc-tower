# popmux: Claude Code Session Control Tower
...
**popmux** (Claude Code Control Tower)
...
**핵심 가치:** popmux는 프로세스가 살아있는지가 아니라, Claude가 **무엇을 생각하고 있는지**를 보여준다.
htop은 CPU를 보여주지만, popmux는 "DB 마이그레이션 테스트 수정 중 — 9개 중 8개 통과"를 보여준다.
...
**popmux를 통한 세션 생성 시 tmux 보장:**

```bash
# popmux new 는 항상 tmux pane에서 Claude를 실행
popmux new --cwd ~/workspace/my-app --label "feature"
...
`popmux new`로 생성한 세션은 항상 tmux 안에서 실행되므로 Full 기능이 보장됨.
...
popmux 첫 실행 시 hook 플러그인 미설치 감지 → 자동 설치 안내:

```
╭─ popmux ──────────────────────────────────────────────╮
│                                                          │
│  ⚠ popmux hook plugin이 설치되지 않았습니다.          │
│                                                          │
...
설치 명령: `popmux install-hooks`
- `~/.claude/plugins/popmux/hooks/hooks.json` 생성
- `~/.claude/plugins/popmux/plugin.json` 생성 (플러그인 매니페스트)
...
- `popmux watch <pane_id>` — 특정 tmux pane 모니터링 시작
- `popmux label <session> "migration work"` — 세션에 별칭 부여
- `popmux tag <session> backend urgent` — 태그 부여
...
popmux는 이를 **primary signal**로 활용하고, JSONL/프로세스를 fallback으로 사용.

##### Primary: Claude Code Hooks (지연 ~5ms (shell sender), 이벤트 드리븐)

popmux가 Claude Code 플러그인으로 hook을 등록하면,
**모든 상태 전이를 push로 즉시 수신** 가능:

```
Hook Event               →  상태 전이           →  popmux 동작
─────────────────────────────────────────────────────────────────
...
```json
// ~/.claude/plugins/popmux/hooks/hooks.json
// 플러그인 hook 포맷: 이벤트명을 키로 하는 map 구조
// (검증: OMC, hookify, ralph-loop 등 실제 플러그인과 동일 포맷)
{
  "description": "popmux session monitoring hooks",
  "hooks": {
    "SessionStart": [{
      "hooks": [{ "type": "command", "command": "popmux hook session-start", "timeout": 3 }]
    }],
    "UserPromptSubmit": [{
      "hooks": [{ "type": "command", "command": "popmux hook user-prompt", "timeout": 3 }]
    }],
    "PreToolUse": [{
      "hooks": [{ "type": "command", "command": "popmux hook pre-tool", "timeout": 3 }]
    }],
    "PostToolUse": [{
      "hooks": [{ "type": "command", "command": "popmux hook post-tool", "timeout": 3 }]
    }],
    "SubagentStart": [{
      "hooks": [{ "type": "command", "command": "popmux hook agent-start", "timeout": 3 }]
    }],
    "SubagentStop": [{
      "hooks": [{ "type": "command", "command": "popmux hook agent-stop", "timeout": 3 }]
    }],
    "Stop": [{
      "hooks": [{ "type": "command", "command": "popmux hook stop", "timeout": 3 }]
    }],
    "SessionEnd": [{
      "hooks": [{ "type": "command", "command": "popmux hook session-end", "timeout": 3 }]
    }]
  }
}
```
...
```bash
#!/bin/sh
# popmux hook sender (lightweight shell script)
PAYLOAD="{\"event\":\"$1\",\"sid\":\"$CLAUDE_SESSION_ID\",\"ts\":$(date +%s%3N)}"
SOCK="${XDG_RUNTIME_DIR:-/tmp}/popmux.sock"
...
**Hook → popmux 통신 (Fire-and-Forget):**

```
Hook command 실행 시:
1. popmux hook <event> 가 호출됨
2. 환경변수에서 세션 정보 추출 ($CLAUDE_SESSION_ID, $CLAUDE_CWD 등)
3. stdin으로 hook context (tool name, prompt 등) 수신
4. Unix socket (${XDG_RUNTIME_DIR:-/tmp}/popmux.sock) 으로 전송 시도
5. popmux가 돌고 있으면 → 수신, 상태 업데이트
   popmux가 안 돌고 있으면 → 연결 실패, 이벤트 유실 (OK)
6. hook command는 즉시 종료 (socket 연결 실패도 exit 0)
```

**설계 원칙: No Daemon, No Persistence**

- popmux는 **TUI 프로세스 하나**만 존재. 별도 daemon 없음.
- Hook 이벤트는 popmux가 안 돌고 있으면 **유실되며, 이는 의도된 동작**.
- popmux 시작 시 `sessions/*.json` + JSONL 끝부분 스캔으로 **현재 상태를 복원**.
  과거 이벤트 이력은 필요 없고, "지금 어떤 상태인가"만 알면 됨.

**popmux 시작 시 상태 복원 (Cold Start):**
```
...
이렇게 하면 popmux를 언제 켜든 즉시 현재 상태를 파악하고,
...
| **전제조건** | popmux 플러그인 설치 | JSONL 파일 접근 가능 | PID만 알면 됨 |
...
```
세션 발견 시:
  if popmux plugin 활성 → Hook mode (Primary)
...
터미널 기반 대시보드 (TUI). 실행: `popmux` 또는 `popmux dashboard`.
...
│                                                          │
│  ⚠ tmux 환경이 아닙니다.                                │
│  popmux는 tmux 안에서 실행해야 Peek/Send가 동작합니다.│
│                                                          │
│  모니터링 전용 모드로 시작합니다.                        │
│                                                          │
╰──────────────────────────────────────────────────────────╯
...
- popmux 시작 시 `state.json`을 미리 로드
...
tmux new-session -d -s _popmux_peek -t ${current_session}

# 2. popup 안에서 그룹 세션에 attach + 대상 window 선택
tmux display-popup -E -w 80% -h 80% \
  "tmux attach -t _popmux_peek \; select-window -t :${target_window_index}"

# 3. popup 닫힐 때 (-E) 그룹 세션 자동 정리
tmux kill-session -t _popmux_peek 2>/dev/null
```
...
**전제: popmux TUI가 실행 중일 때만 알림이 동작한다.**
popmux가 꺼져 있으면 알림 없음 (의도된 동작).
...
**알림 내용:**
```
[popmux] ✓ migration-api completed (2m 34s)
...
- popmux 종료 시(`q` 키, SIGINT, SIGTERM) `process.exit(0)` 호출 전
...
- `popmux new` vs 사용자가 직접 tmux + claude 실행 후 자동 탐지에 의존
...
```
popmux history [--project <name>] [--since <date>]
```
...
```
popmux group create "sprint-23" --sessions 1,2,4
popmux group list
```
...
- popmux 크래시 시 모니터링 대상 세션에 영향 없음 (read-only 원칙)
...
popmux 자체 로그에서 측정 가능한 지표로 정의.

| 지표 | 정의 | 목표 |
|------|------|------|
| **일일 활성 세션 수** | popmux가 한 번이라도 추적한 세션 수/일 | ≥ 3 |
...
popmux 종료 시 `~/.local/share/popmux/usage.json`에 세션 통계를 기록하여 자체 측정 가능.
...
```
  ...
  └──────┬───────┘  └──────┬───────┘  └──────────────┘
         ...
         ▼                   ▼                   │
  ┌──────────────────────────────────────────────┤
  │         ${XDG_RUNTIME_DIR:-/tmp}/popmux.sock (Unix Socket)     │
  └──────────────────┬───────────────────────────┘
                     │
  ┌──────────────────▼──────────────────────────────────┐
  │              popmux (TUI process)                         │
...
```
1. [Hook] Claude Code가 상태 전이 시 popmux hook <event> 실행
2. [Parse] stdin + 환경변수에서 sessionId, event type, context 추출
3. [Send] Unix socket (${XDG_RUNTIME_DIR:-/tmp}/popmux.sock)으로 HookEvent 전송
4. [Receive] popmux TUI 프로세스의 Hook Receiver가 수신
...
1. [Create] tmux new-session -d -s _popmux_peek -t ${current_session}
2. [Popup] tmux display-popup -E -w 80% -h 80% \
     "tmux attach -t _popmux_peek \; select-window -t :${target_window}"
3. [Interact] 실제 pane이므로 직접 타이핑 가능 (native stdin)
4. [Close] Esc/detach → popup 닫힘 → kill-session _popmux_peek
...
# 세션 관리
popmux list                     # 활성 세션 목록 (non-interactive)
popmux list --json              # JSON 출력
popmux status <session>         # 특정 세션 상태
popmux watch <pane_id>          # 수동으로 pane 모니터링 추가
popmux label <session> "name"   # 별칭 부여
popmux tag <session> <tags...>  # 태그 부여

# 명령 전달
popmux send <session> "message" # 세션에 메시지 전송 (send-keys)
popmux peek <session>           # tmux popup으로 pane 미리보기

# 세션 생성 (Phase 1.5)
# popmux new [--cwd <path>] [--label <name>]  # 미결 — Phase 1.5에서 설계

# 히스토리
popmux history [--project <name>] [--since <date>]

# 그룹
popmux group create <name> [--sessions <ids>]
popmux group list

# 설정
popmux config                   # 설정 편집
popmux config set <key> <value>

# 상태 확인 (non-interactive, 스크립트용)
popmux status                   # 전체 세션 상태 한 줄씩 출력 후 종료
```

### 8.2 Global Options

```
--config <path>     설정 파일 경로 (기본: ~/.config/popmux/config.yaml)
--no-color          컬러 비활성화
...
history:
  ...
  store_path: "~/.local/share/popmux/history"
...
popmux 재시작 시:
- 기존 세션 정보를 `~/.local/share/popmux/state.json`에서 복원
...
- popmux 시작 시: `ssh -fN -R ${XDG_RUNTIME_DIR:-/tmp}/popmux.sock:${XDG_RUNTIME_DIR:-/tmp}/popmux.sock <host>`
- 원격 Claude hook이 동일한 소켓 경로로 전송 → 이벤트가 로컬 popmux로 터널링
- 원격 서버에 hook 플러그인 설치 필요: `popmux install-hooks --remote <host>`
- 레이턴시: ~5ms + 네트워크 (~50ms)
- 로컬 경험과 동일
...
# 내부 동작:
# 1. scp hooks/ 디렉토리를 원격:~/.claude/plugins/popmux/ 로 복사
# 2. 검증: ssh <host> "ls ~/.claude/plugins/popmux/hooks/hooks.json"
```
...
- [ ] `popmux` 실행 시 모든 활성 Claude Code 세션을 자동 탐지
...
- [ ] 작업 완료 시 데스크톱 알림 발생
- [ ] `popmux list --json` 으로 스크립트 연동 가능
- [ ] popmux TUI 실행 중일 때 작업 완료 알림 동작 확인
...
popmux는 "Claude Code 세션"이라는 특정 도메인에 특화된 모니터링 + 제어 도구.
...
| `~/.config/popmux/config.yaml` | popmux 설정 | Read/Write |
| `~/.local/share/popmux/state.json` | 세션 메타 (label, tag) | Read/Write |
| `~/.local/share/popmux/history/` | 히스토리 데이터 | Read/Write |
