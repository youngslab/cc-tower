...
**Hook sender 성능 고려:**
위 hook command(`cc-tower hook <event>`)가 Node.js로 실행되면 ~100-150ms 시작 오버헤드.
경량 shell script로 구현하여 레이턴시를 ~5ms로 낮추는 것을 권장:
```bash
#!/bin/sh
# cc-tower hook sender (lightweight)
echo "{\"event\":\"$1\",\"sid\":\"$CLAUDE_SESSION_ID\",\"ts\":$(date +%s%3N)}" \
  | socat - UNIX-CONNECT:${XDG_RUNTIME_DIR:-/tmp}/cc-tower.sock 2>/dev/null
exit 0
```
...
├─ claude --print -p "Summarize in 1 sentence: <recent JSONL>"
├─ 최근 N개 메시지만 전달 (토큰 절약)
├─ 결과를 캐시하여 동일 턴 재요약 방지
...
│                                                                     │
│  ─────────────────────────────────────────────────────────────────  │
│  [Enter] Detail  [z] Zoom  [p] Peek  [/] Send  [n] New  [q] Quit  │
...
│                                                          │
│  ⚠ tmux 환경이 아닙니다.                                │
│  cc-tower는 tmux 안에서 실행해야 Peek/Zoom이 동작합니다.│
│                                                          │
│  모니터링 전용 모드로 시작합니다.                        │
│                                                          │
╰──────────────────────────────────────────────────────────╯
...
│                                                                     │
│  [/] Send  [z] Zoom  [p] Peek  [b] Back                           │
└─────────────────────────────────────────────────────────────────────┘
```

#### FR-8: 3-Tier Pane Interaction (Peek / Zoom / Send)

대시보드에서 Claude Code 세션에 접근하는 세 가지 depth:
...
- 그룹 세션에서 zoom 하면 원본에도 반영됨
  → popup에서는 zoom 불필요 (popup 크기가 곧 뷰 크기)

**Peek에서 직접 입력이 가능하므로**, send-keys 프록시가 필요 없다.
간단한 명령은 Peek에서 바로 타이핑, 본격적인 작업은 Enter로 Zoom 에스컬레이션.

##### Tier 3: Zoom (실제 pane 전체화면, 직접 상호작용)

`z` 키 → `tmux select-pane + resize-pane -Z`로 대상 pane을 전체화면 zoom.
pane을 이동하지 않고, 실제 Claude pane에서 직접 작업.
...
cc-tower가 zoom 진입 시 tmux 키바인딩을 자동 등록:
```bash
# zoom 진입 시
TOWER_WINDOW=$(tmux display-message -p '#{window_id}')
TOWER_PANE=$(tmux display-message -p '#{pane_id}')

tmux bind-key -n F12 \
  "resize-pane -Z; \
   select-window -t @${TOWER_WINDOW}; \
   select-pane -t ${TOWER_PANE}"
```

다른 window에 있는 pane도 동일하게 동작:
```bash
# 다른 window의 Claude pane으로 zoom
tmux select-window -t ${target_window}
tmux select-pane -t ${target_pane}
tmux resize-pane -Z

# F12 → unzoom + tower window/pane으로 자동 복귀
```

**장점:**
- 실제 pane이므로 100% 실시간, 캡처 지연 없음
- 캡처/렌더링/send-keys 프록시 전부 불필요
- Claude와 직접 상호작용 (native input)
- pane 이동 없음, 레이아웃 보존
...
| `z` | **Zoom** | `select-pane` + `resize-pane -Z` | 전체화면 작업 | 떠남 (F12 복귀) |

**키 바인딩 정리:**
- `Enter` → Detail (TUI 내 상세 뷰)
- `/` → Send (대시보드 안 떠남, 한 줄 전달)
- `p` → Peek (popup 오버레이, 실제 pane 라이브 + 직접 타이핑)
- `z` → Zoom (전체화면 전환, 완전한 네이티브 경험, F12 복귀)

**에스컬레이션 흐름:** Detail → Peek → Zoom (점진적 몰입)
...
| 해당 세션이 zoom/peek 중이 아님 | 자동 감지 | 직접 보고 있으면 불필요 |
...
notifications:
  ...
  suppress_when_focused: true # 대시보드/peek/zoom 중이면 알림 안 함
  channels:
    desktop: true
    tmux_bell: true
    sound: false
...
| **알림→행동 전환** | 완료 알림 후 5분 내 Send/Peek/Zoom 사용 비율 | ≥ 50% |
| **Interaction 사용률** | Send/Peek/Zoom 중 하나라도 사용한 세션 비율 | ≥ 30% |
...
5. [Escalate] popup 안에서 별도 키로 Zoom 전환 가능

[Zoom] (z 키)
1. [Record] 대시보드의 window_id + pane_id 기록
2. [Bind] F12 복귀 키바인딩 등록:
     tmux bind-key -n F12 "resize-pane -Z; select-window -t @W; select-pane -t P"
3. [Navigate] 대상이 다른 window면 select-window 먼저 수행
4. [Zoom] tmux select-pane -t <target> && tmux resize-pane -Z
5. [Interact] 사용자가 실제 Claude pane에서 직접 작업
6. [Return] F12 → unzoom + 대시보드 window/pane으로 자동 복귀
7. [Cleanup] 복귀 후 F12 키바인딩 해제
```
...
cc-tower zoom <session>           # tmux zoom으로 pane 전체화면 (F12 복귀)
...
| **W4** | Interaction + Polish | Send/Peek/Zoom 3-tier, Config, 테스트 |
...
- [ ] 대시보드에서 tmux popup으로 세션 미리보기 가능 (Peek: `p` 키)
- [ ] 대시보드에서 세션 pane으로 zoom 전환 + F12 복귀 가능 (Zoom: `Enter` 키)
- [ ] 작업 완료 시 데스크톱 알림 발생
