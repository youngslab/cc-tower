 ...
 */
function isClaudeSessionInfo(val: unknown): val is ClaudeSessionInfo {
 ...
 */
export function isHeadlessSession(sessionsDir: string, pid: number): boolean {
  try {
 ...
 */
export async function parseSessionFile(filePath: string): Promise<ClaudeSessionInfo | null> {
  let raw: string;
  try {
...
export function scanProcesses(
  claudeDir: string,
  usedSessionIdsByCwd: (cwd: string, pid: number) => Set<string>,
): ClaudeSessionInfo[] {
  const active: ClaudeSessionInfo[] = [];
  try {
