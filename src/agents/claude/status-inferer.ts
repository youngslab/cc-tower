 ...
 */
export function coldStartScan(jsonlPath: string): SessionState {
  let content: string;
  try {
 ...
 */
export function coldStartLastTask(jsonlPath: string): string | undefined {
  let content: string;
  try {
    const stat = fs.statSync(jsonlPath);
    const readSize = Math.min(stat.size, 65536); // last 64KB is enough
 ...
 */
export function coldStartCustomTitle(jsonlPath: string): string | undefined {
  try {
