...
export const RUNNING_STATUSES = new Set<Session['status']>(['thinking', 'executing', 'agent']);
...
export function computeNeedsAttention(
  ...
  if (RUNNING_STATUSES.has(prevStatus) && nextStatus === 'idle') return true;
  if (RUNNING_STATUSES.has(nextStatus)) return false;
  return undefined;
}
