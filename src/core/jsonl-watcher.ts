...
export class JsonlWatcher extends EventEmitter {
  ...
  watch(sessionId: string, jsonlPath: string): void {
    ...
    const fsWatcher = fs.watch(jsonlPath, () => {
      ...
      if (length <= 0) { entry.offset = newSize; return; } // file truncated or rewound
      const fd = fs.openSync(jsonlPath, 'r');
   ...
   */
  async readEarlyContext(jsonlPath: string, maxMessages: number = 15): Promise<string | undefined> {
    try {
      const stat = fs.statSync(jsonlPath);
