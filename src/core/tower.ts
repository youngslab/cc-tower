...
export class Tower extends EventEmitter {
  ...
  private rehydrateFromState(): void {
    ...
    const panePidMap = new Map<number, string>(); // shell_pid → paneId
    ...
    const claudePanes = new Set<string>();
    try {
      ...
      for (const line of out.trim().split('\n').filter(Boolean)) {
        ...
        if (paneId) livePanes.add(paneId);
        if (paneId && pid) panePidMap.set(parseInt(pid), paneId);
        if (paneId && cmd && /claude/i.test(cmd)) claudePanes.add(paneId);
      }
    } catch {
      livePanes = new Set();
    }
    ...
    for (const [identity, inst] of this.store.getPersistedInstanceEntries()) {
      ...
      if (!isRemote && !(paneId && claudePanes.has(paneId))) continue;
      ...
      let rehydrateUsedConvId: string | undefined;
      try {
        const claudeDir = this.config.discovery.claude_dir.replace('~', os.homedir());
        const sessionsDir = path.join(claudeDir, 'sessions');
      ...
      try {
        ...
        if (task) this.store.update(sessionIdentity(session), { currentTask: task });
      } catch (err) {
  ...
  private rehydrateNewSessions(livePanes: Set<string>, panePidMap: Map<number, string>): void {
    const claudeDir = this.config.discovery.claude_dir.replace('~', os.homedir());
    const sessionsDir = path.join(claudeDir, 'sessions');
    ...
    const registeredSids = new Set(
      this.store.getAll().filter(s => s.paneId && livePanes.has(s.paneId)).map(s => s.sessionId)
    );
    ...
    for (const f of files) {
      try {
        ...
        if (!pid || !sid || !cwd) continue;
        if (kind && kind !== 'interactive') continue; // skip headless/sdk sessions
        if (registeredSids.has(sid)) continue;

        const paneId = this.findPaneForPid(pid, panePidMap);
        if (!paneId || !livePanes.has(paneId)) continue;
  ...
  private readLastUserTask(sessionId: string, cwd: string): string | undefined {
    try {
      const claudeDir = this.config.discovery.claude_dir.replace('~', os.homedir());
  ...
  private findPaneForPid(claudePid: number, panePidMap: Map<number, string>): string | undefined {
    let current = claudePid;
    for (let depth = 0; depth < 12; depth++) {
      if (panePidMap.has(current)) return panePidMap.get(current);
      try {
        const status = fs.readFileSync(`/proc/${current}/status`, 'utf8');
        const m = status.match(/^PPid:\s+(\d+)/m);
        if (!m) break;
        const ppid = parseInt(m[1]);
        if (ppid <= 1 || ppid === current) break;
        current = ppid;
      } catch { break; }
    }
    return undefined;
   ...
   */
  private reconcilePaneTitles(): void {
  ...
  private applyPaneTitles(titles: Map<string, string>): void {
    for (const session of this.store.getAll()) {
      ...
      if (title === undefined) continue; // not a local pane (e.g. remote mirror) — leave to hooks
      const busy = isPaneBusy(title);
      if (busy === undefined) continue; // not a recognized Claude Code glyph — leave untouched
   ...
   */
  private async refreshAllSummaries(identity: string, jsonlPath: string): Promise<void> {
    ...
    try {
      ...
      if (result) {
        if (result.goal) this.store.updateMeta(identity, { goalSummary: result.goal });
        if (result.context) this.store.updateMeta(identity, { contextSummary: result.context });
        if (result.nextSteps) this.store.updateMeta(identity, { nextSteps: result.nextSteps });
    ...
    if (session) {
      ...
      if (hookSid !== 'unknown' && hookSid !== session.sessionId && isDirectChild) {
        ...
        const staleSidResolve = this.resolver.claim({
          ...
          hookEvent: event.event as 'user-prompt' | 'stop' | 'pre-tool' | 'post-tool',
        });
          ...
          ? path.basename(this.jsonlPaths.get(session.sessionId)!, '.jsonl') : session.sessionId;
        {
          ...
          this.ledger?.append({
            ...
            to: chosenConvId,
            trigger: event.event as string, confidence: staleSidResolve.confidence, reason: staleSidResolve.reason,
            claimTableSize: this.resolver.snapshotClaimTable().size,
        ...
        if (fs.existsSync(newJsonl)) {
          ...
          if (identity) this.store.setInstanceConversationId(identity, path.basename(newJsonl, '.jsonl'));
          ...
          try {
            ...
            const dirWatcher = fs.watch(dir, (evt, filename) => {
              ...
              if (filename === expectedFile && fs.existsSync(newJsonl)) {
                ...
                this.jsonlPaths.set(hookSid, newJsonl);
                this.jsonlWatcher.watch(hookSid, newJsonl);
                if (capturedIdentity) this.store.setInstanceConversationId(capturedIdentity, path.basename(newJsonl, '.jsonl'));
                const customTitle = agents.claude.extractLabel(newJsonl);
                if (customTitle) {
