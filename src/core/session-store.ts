...
export class SessionStore extends EventEmitter {
   ...
   */
  getAllResumableSessions(scanned: ScannedSession[], scanComplete = false): Array<{ sessionId: string; cwd: string; startedAt: number; label?: string; goalSummary?: string; contextSummary?: string; sshTarget?: string }> {
    const merged = new Map<string, { sessionId: string; cwd: string; startedAt: number; label?: string; goalSummary?: string; contextSummary?: string; sshTarget?: string }>();
    ...
    for (const [sessionId, entry] of this.persistedMeta) {
      ...
      if (scanComplete && !base && !entry.sshTarget) continue;
      const cwd = base?.cwd ?? entry.cwd;
      if (!cwd) continue; // can't resurrect without a cwd
