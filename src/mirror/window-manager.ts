...
export function windowName(host: string, pane: string): string {
 ...
 */
export function parseWindowName(name: string): { host: string; pane: string } | null {
  if (!name.startsWith('mirror-')) return null;
...
async function listMirrorWindowNames(): Promise<string[]> {
...
async function listPaneInfoForWindow(win: string): Promise<Array<{ dead: boolean; pid: number }>> {
  const r = await execa('tmux', [
    'list-panes', '-t', `${MIRROR_SESSION}:${win}`, '-F', '#{pane_dead}|#{pane_pid}',
  ], { reject: false });
  if (r.exitCode !== 0) return [];
  const out: Array<{ dead: boolean; pid: number }> = [];
  for (const line of r.stdout.split('\n')) {
    if (!line) continue;
    const [deadStr, pidStr] = line.split('|') as [string, string];
...
export async function ensureMirrorSession(): Promise<void> {
 ...
 */
export async function isMirrorAlive(host: string, pane: string): Promise<boolean> {
  const win = windowName(host, pane);
  const winNames = await listMirrorWindowNames();
  if (!winNames.includes(win)) return false;

  const panes = await listPaneInfoForWindow(win);
  if (panes.length === 0) return false;
  for (const p of panes) {
    if (p.dead) return false;
    if (!await isSshAlive(p.pid)) return false;
  }
  return true;
}
 ...
 */
export async function cleanupDeadMirrors(): Promise<string[]> {
  const winNames = await listMirrorWindowNames();
  const killed: string[] = [];
 ...
 */
export async function cleanupStaleMirrors(opts?: { ttlMs?: number }): Promise<string[]> {
 ...
 */
function buildSshCommand(target: MirrorTarget): string {
 ...
 */
export async function goToMirror(target: MirrorTarget, opts?: { ttlMs?: number }): Promise<void> {
